    ...
    <main className="agent-shell">
      ...
      {context && <>
        <section className="copilot-grid">
          <article className="agent-panel context-summary"><div className="panel-title"><span className="section-kicker">AI CONTEXT SUMMARY</span><span className="status-pill">Event 근거 {interpretation?.evidence.length ?? 0}건</span></div><h2>{interpretation?.summary ?? "Journey Context를 해석하는 중입니다."}</h2><p>현재 단계: {labels[context.payload.current_step] ?? context.payload.current_step} · 오류: {context.payload.error_codes.join(", ") || "없음"} · 재시도: {context.payload.retry_count}회</p><div className="intent-tag">Intent · {interpretation?.customer_intent} ({Math.round((interpretation?.intent_confidence ?? 0) * 100)}%)</div></article>
